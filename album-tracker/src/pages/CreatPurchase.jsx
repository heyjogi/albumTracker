...
export default function CreatePurchase() {
    ...
    const fetchOptions = async () => {
        ...
        if (ts && ts.length > 0) {
            ...
            setForm(f => ({ ...f, team_id: firstTeam.public_team_id, team_name: firstTeam.name }));
    ...
    const filteredDisplayMembers = useMemo(() => {
        ...
        return albumMembers.filter(am => teamMemberNames.includes(am.member_name));
    ...
    const handleTeamChange = async (e) => {
        ...
        if (teamId) {
            // [수정] 전체 팀원이 아니라 '내'가 담당하는 멤버만 가져옵니다.
    ...
    const submit = async () => {
        ...
        try {
            ...
            const filteredMembers = albumMembers.filter(m => selectedMemberIds.includes(m.id));
            ...
            if (form.team_id) {
                const selectedTeam = teams.find(t => t.public_team_id === form.team_id);
        ...
        <div className="cp-wrapper">
            ...
            <div className="cp-card">
                ...
                <div>
                    ...
                    <div className="cp-select-wrap">
                        <select className={selectClass} value={form.store_id} onChange={handleStoreChange}>
                            ...
                            {stores.map(s => (
                                <option key={s.public_store_id} value={s.public_store_id}>{s.name}</option>
                            ))}
                        </select>
                ...
                <div>
                    ...
                    <div className="cp-select-wrap">
                        <select className={selectClass} value={form.team_id} onChange={handleTeamChange}>
                            ...
                            {teams.map(t => (
                                <option key={t.public_team_id} value={t.public_team_id}>{t.name}</option>
                            ))}
                        </select>
                        <span className="cp-select-icon">▾</span>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="cp-member-scroll">
                            ...
                            {filteredDisplayMembers.map(m => {
                                ...
                                cardClassName += isSelected ? ' cp-member-card-on' : ' cp-member-card-off';
                                ...
                                return (
                                    ...
                                    >
                                        <div className="cp-member-img">
                                            ...
                                            {m.event_image_url ? (
                                                <img src={m.event_image_url} alt={m.member_name} className="cp-member-img-photo" />
                                            ) : (
                                                <span className="cp-member-img-placeholder">{m.member_name?.charAt(0)}</span>
                                            )}
                                        </div>
                ...
                <div className="space-y-4">
                    ...
                    {/* 계산 미리보기 안내 */}
                    {!isPersonal && selectedMemberIds.length > 0 && (
                        <div className="bg-brand-50 p-3 rounded-xl border border-brand-100">
                            <p className="text-[12px] text-brand-700 font-medium">
                                ✨ 인당 예상 금액 (총 {selectedMemberIds.length}명)
                            </p>
                            <div className="flex justify-between text-[11px] text-brand-600 mt-1">
                                <span>배송비: +{Math.floor((form.shipping_fee || 0) / selectedMemberIds.length).toLocaleString()}원</span>
                                <span>할인: -{Math.floor((form.discount || 0) / selectedMemberIds.length).toLocaleString()}원</span>
                                <span className="font-bold">합계: {(
                                    Number(form.price) +
                                    Math.floor((form.shipping_fee || 0) / selectedMemberIds.length) -
                                    Math.floor((form.discount || 0) / selectedMemberIds.length)
